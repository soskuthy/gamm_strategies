...
for (i in 1:n_speakers) {
  ...
  f2_end.speaker <- rnorm(1, f2_end_mean, f2_end_sd.speaker)
  ...
  for (j in 1:n_trajectories_per_speaker) {
    ...
    f2_end <- rnorm(1, f2_end.speaker, f2_end_sd.traj)
    x0 <- rnorm(1, x0.speaker, x0_sd.traj)
    k <- rnorm(1, k.speaker, k_sd.traj)
    ys_m[,(i-1)*n_trajectories_per_speaker + j] <- ((f2_end - f2_start) / (1 + exp(-k*(xs-x0)))) + f2_start + rnorm(length(xs), 0, noise_sd)
  }
}
